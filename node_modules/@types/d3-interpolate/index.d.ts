 ...
 * Then, for the given parameter `t`, the templateâ€™s embedded interpolators are evaluated. The updated array template is then returned.
 *
 ...
 * based on [â€œSmooth and efficient zooming and panningâ€](http://www.win.tue.nl/~vanwijk/zoompan.pdf).
...
/**
 * Returns a uniform nonrational B-spline interpolator through the specified array of colors, which are converted to RGB color space.
 * The control points are implicitly repeated such that the resulting spline has cyclical CÂ² continuity when repeated around `t` in [0,1];
 ...
 * they will be converted to HSL using `d3.hsl`. If either colorâ€™s hue or saturation is NaN, the opposing colorâ€™s channel value is used.
 ...
 * they will be converted to HCL using `d3.hcl`. If either colorâ€™s hue or chroma is NaN, the opposing colorâ€™s channel value is used.
 * The shortest path between hues is used. The return value of the interpolator is an RGB string.
 */
 ...
 * If either colorâ€™s hue or saturation is NaN, the opposing colorâ€™s channel value is used. The shortest path between hues is used. The return value of the interpolator is an RGB string.
 */
...
/**
 * Returns a uniform nonrational B-spline interpolator through the specified array of `values`, which must be numbers.
 * The control points are implicitly repeated such that the resulting one-dimensional spline has cyclical CÂ² continuity when repeated around `t` in [0,1].
