...
# Use basic logging - NO UNICODE CHARACTERS for Windows compatibility
logging.basicConfig(
    level=logging.INFO,
...
def lazy_import_components_sync():
    """Synchronous lazy import of components."""
    global scanner_registry, scanner_engine, plugin_manager, components_loaded, loading_in_progress
    
    if components_loaded or loading_in_progress:
...
async def configure_engine_async():
    """Configure scanner engine asynchronously."""
    global scanner_engine, scanner_registry
    
    if scanner_engine and scanner_registry:
        try:
            ...
            # Load essential scanners in background
            asyncio.create_task(load_scanners_background())
            
        except asyncio.TimeoutError:
            logger.warning("Scanner engine configuration timed out")
        except Exception as e:
            logger.error(f"Scanner engine configuration failed: {e}")

async def load_scanners_background():
    """Load scanners in background without blocking."""
    try:
        if scanner_registry and hasattr(scanner_registry, 'load_scanners'):
            await asyncio.wait_for(
                scanner_registry.load_scanners(lazy_load=True, preload_essential=True),
                timeout=30.0
            )
            scanner_count = len(scanner_registry.get_all_scanners())
            logger.info(f"Background scanner loading complete: {scanner_count} scanners ready")
    except asyncio.TimeoutError:
        logger.warning("Background scanner loading timed out")
    except Exception as e:
        logger.error(f"Background scanner loading failed: {e}")
...
async def api_ready():
    ...
    global scanner_registry, scanner_engine, plugin_manager, components_loaded
    ...
    logger.info(f"API ready check - Ready: {backend_ready}, Components: {components_loaded}, Scanners: {scanner_count}")
    return status_info
